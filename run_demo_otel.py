...
exercise GalileoLogger and its registered Agent Control bridge; this variant uses
...
from common import resolve_agent_control_api_key, resolve_agent_control_api_key_header
...
def _otel_endpoint(args: Any) -> str:
    ...
    api_url = args.api_base_url or _required_env("GALILEO_API_URL")
    return urljoin(api_url.rstrip("/") + "/", "otel/traces").rstrip("/")
...
def _otel_headers(args: Any) -> dict[str, str]:
    ...
    if configured:
        try:
            ...
            raise RuntimeError("AGENT_CONTROL_OTEL_HEADERS must be a JSON object.") from exc
        if not isinstance(value, dict) or not all(isinstance(k, str) and isinstance(v, str) for k, v in value.items()):
            raise RuntimeError("AGENT_CONTROL_OTEL_HEADERS must contain only string keys and values.")
        return value

    return {
        "Galileo-API-Key": _required_env("GALILEO_API_KEY"),
        "project": args.project,
        "logstream": args.log_stream,
    }


def _validate_otel_configuration(args: Any, endpoint: str, headers: dict[str, str]) -> None:
    required_headers = {"Galileo-API-Key", "project", "logstream"}
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError("AGENT_CONTROL_OTEL_ENDPOINT must be an absolute HTTP(S) URL.")


def _resolve_galileo_ids(args: Any) -> tuple[str, str]:
    from galileo.log_stream import LogStream

    log_stream = LogStream.get(name=args.log_stream, project_name=args.project)
    if log_stream is None or not log_stream.id or not log_stream.project_id:
        raise RuntimeError(
            f"Could not resolve Galileo log stream {args.project}/{args.log_stream}. "
            "Create it first or correct GALILEO_PROJECT and GALILEO_LOG_STREAM."
        )
    return str(log_stream.project_id), str(log_stream.id)
...
def _set_common_span_attributes(span: Any, *, operation: str, input_value: Any) -> None:
    ...
    span.set_attribute("gen_ai.input.messages", json.dumps([{"role": "user", "content": input_value}], default=str))
...
async def _verify_otel_trace(
    ...
    *,
    ...
    api_key = _required_env("GALILEO_API_KEY")
    ...
    raw_control_events = _count_span_name(trace_payload, "agent_control.control_execution")
    if raw_control_events:
        raise RuntimeError(
            f"OTLP delivery succeeded, but Galileo ingest classified {raw_control_events} "
    ...
    raise RuntimeError("The OTEL trace was stored, but it contained no Agent Control execution spans.")


async def _run(args: Any) -> None:
    demo._configure_galileo_environment(args)
    ...
    tracer = provider.get_tracer("agent-control-galileo-otel-demo")
    ...
    if not args.skip_runtime_token_check and args.runtime_auth_mode == "jwt":
        await demo._verify_runtime_token_exchange(args, target_type=target_type, target_id=target_id)
    ...
    luna_config = await demo._verify_bound_controls(args, target_type=target_type, target_id=target_id)
    ...
    print()
    print("Telemetry path: OTLP/HTTP only (GalileoLogger is not initialized)")
    ...
    print(f"Galileo log_stream_id: {log_stream_id}")
    ...
    try:
        with tracer.start_as_current_span("agent-control-galileo-otel-e2e") as root_span:
            _set_common_span_attributes(root_span, operation="workflow", input_value=trace_input)
            ...
            if skip_luna_control:
                ...
                with tracer.start_as_current_span(demo.draft_transfer_plan.__name__) as llm_span:
                    _set_common_span_attributes(llm_span, operation="chat", input_value=args.prompt)
                    llm_span.set_attribute("gen_ai.request.model", "demo-rule-based")
                    try:
                        draft_response = await demo.draft_transfer_plan(args.prompt, transfer)
            ...
            if blocked_output is None:
                ...
                for attempt in range(1, args.max_steer_attempts + 1):
                    with tracer.start_as_current_span(demo.process_wire_transfer.__name__) as tool_span:
                        _set_common_span_attributes(tool_span, operation="execute_tool", input_value=tool_input)
                        tool_span.set_attribute("gen_ai.tool.name", demo.process_wire_transfer.__name__)
                        ...
                        try:
                            ...
                            blocked_output = demo._control_exception_message(f"tool attempt {attempt}", exc)
                            break
            ...
            if blocked_output is None:
                final_answer = demo._render_final_answer(draft_response, tool_output, steering_history)
            else:
    ...
    if args.query_trends:
        if not args.verify_api:
            await asyncio.sleep(args.verify_delay_seconds)
        await demo._query_trends(args, project_id=project_id, log_stream_id=log_stream_id)
