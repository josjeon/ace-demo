...
from setup_controls import control_specs, ensure_demo_controls
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a standalone demo agent and let real Agent Control execution events flow into Galileo."
    )
    parser.add_argument(
        "--prompt",
    ...
    parser.add_argument("--amount", type=float, default=None, help="Override parsed transfer amount.")
    parser.add_argument("--destination-country", default=None, help="Override parsed transfer destination country.")
    parser.add_argument("--recipient-name", default=None, help="Override parsed transfer recipient name.")
    parser.add_argument("--fraud-score", type=float, default=None, help="Override computed fraud score.")
    parser.add_argument("--max-steer-attempts", type=int, default=3, help="Maximum steering retries for the transfer.")
    parser.add_argument("--project", default=DEFAULT_PROJECT, help="Galileo project name.")
    parser.add_argument("--log-stream", default=DEFAULT_LOG_STREAM, help="Galileo log stream name.")
    parser.add_argument("--mode", choices=("batch", "distributed"), default="batch", help="Galileo logger mode.")
    parser.add_argument("--agent-name", default=DEFAULT_AGENT_NAME, help="Agent Control agent name.")
    parser.add_argument("--server-url", default=DEFAULT_AGENT_CONTROL_URL, help="Agent Control server base URL.")
    parser.add_argument(
        "--target-type",
    ...
    parser.add_argument("--session-name", default="agent-control-galileo-e2e", help="Galileo session name.")
    parser.add_argument(
        "--console-url",
        default=DEFAULT_CONSOLE_URL,
        help="Galileo console URL used by the SDK for auth and configuration.",
    )
    parser.add_argument(
        "--api-base-url",
        default=DEFAULT_API_URL,
        help="Galileo API base URL used for devstack verification calls.",
    )
    parser.add_argument(
        "--verify-api",
        action="store_true",
        help="Fetch the stored trace from Galileo after ingestion and print a short summary.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--skip-control-setup",
        action="store_true",
        help="Deprecated no-op. Controls are expected to already be bound to the Galileo log stream.",
    )
    parser.add_argument(
        "--setup-controls",
        action="store_true",
        help="Create/update demo controls and bind them to the Galileo log stream. Off by default.",
    )
    parser.add_argument(
        "--runtime-auth-mode",
...
def _parse_transfer_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    amount = args.amount
    if amount is None:
        amount_match = re.search(r"\$?\s*([\d,]+(?:\.\d+)?)\s*(k)?", prompt, re.IGNORECASE)
        amount = 15000.0
        if amount_match:
    ...
    if destination_country is None:
        country_match = re.search(r"\bin\s+(?:the\s+)?([A-Za-z ]+?)(?:\s+for|\s+from|\.|$)", prompt)
        if country_match is None:
            country_match = re.search(r"\bto\s+(?:the\s+)?([A-Za-z ]+?)(?:\s+for|\s+from|\.|$)", prompt)
        destination_country = country_match.group(1).strip() if country_match else "United States"

    recipient_name = args.recipient_name
    if recipient_name is None:
        recipient_match = re.search(r"\b(?:to|for)\s+([A-Z][A-Za-z0-9 &.-]+?)(?:\s+in|\s+for|\.|$)", prompt)
...
def _compute_fraud_score(transfer: dict[str, Any], override: float | None) -> float:
    ...
    if any(country in destination for country in ("north korea", "iran", "syria", "cuba", "crimea")):
        return 0.95
...
def _collect_control_spans(workflow: Any) -> list[Any]:
    return [span for span in getattr(workflow, "spans", []) if getattr(span, "type", None) == "control"]
...
def _resolve_api_base_url(args: argparse.Namespace) -> str:
    ...
    raise RuntimeError("Missing Galileo API base URL. Set GALILEO_API_URL or pass --api-base-url.")


def _configure_galileo_environment(args: argparse.Namespace) -> None:
    if args.console_url:
...
def _text_filter(name: str, value: str, *, case_sensitive: bool = True) -> dict[str, Any]:
    return {
        "filter": {
            "name": name,
            "operator": "eq",
    ...
    return [
        {
            ...
            "body": {
                ...
                "metric_details": [
                    {"id": "control_name_counts", "metric_name": "control.name", "aggregation": "Count"}
                ],
        ...
        },
        {
            ...
            "body": {
                ...
                "metric_details": [
                    {"id": "tool_control_names", "metric_name": "control.name", "aggregation": "Count"}
                ],
        ...
        },
        {
            ...
            "body": {
                ...
                "metric_details": [
                    {"id": "matched_outcomes", "metric_name": "control.output.matched", "aggregation": "Count"}
                ],
                "filter_tree": _and_filter(
                    _text_filter("type", "control"),
                    _text_filter("agent_name", agent_name),
        ...
        },
        {
            ...
            "body": {
                ...
                "metric_details": [
                    {"id": "selector_paths", "metric_name": "control.selector_path", "aggregation": "Count"}
                ],
                "filter_tree": _and_filter(
                    _text_filter("type", "control"),
                    _text_filter("agent_name", agent_name),
...
def _iter_condition_evaluators(condition: Any) -> list[tuple[str | None, dict[str, Any]]]:
...
def _condition_evaluators(control_data: dict[str, Any]) -> list[tuple[str | None, dict[str, Any]]]:
...
def _values_match(expected: Any, actual: Any) -> bool:
    if isinstance(expected, list):
        return isinstance(actual, list) and {str(value).lower() for value in expected} <= {
            str(value).lower() for value in actual
        }
    if isinstance(expected, dict):
        return isinstance(actual, dict) and all(_values_match(value, actual.get(key)) for key, value in expected.items())
    return expected == actual


def _control_matches_spec(control: dict[str, Any], expected_spec: dict[str, Any]) -> bool:
    ...
    if expected_evaluator_name == "galileo.luna":
        return actual_action.get("decision") in {"observe", "deny"} and "scorer_label" in actual_evaluator_config
...
def _find_bound_control(
    ...
    for control in controls:
        if control.get("name") == expected_name:
            return control
    ...
    for control in controls:
        if _control_matches_spec(control, expected_spec):
            return control
...
async def _verify_bound_controls(
    ...
    *,
    ...
    if not api_key:
        raise RuntimeError("Missing AGENT_CONTROL_API_KEY or GALILEO_API_KEY in the environment.")
    ...
    async with httpx.AsyncClient(base_url=args.server_url.rstrip("/"), headers=headers, timeout=60.0) as client:
        response = await client.get(f"/api/v1/agents/{args.agent_name}/controls", params=params)
        response.raise_for_status()
        controls = response.json().get("controls", [])

    if not isinstance(controls, list):
        raise RuntimeError("Agent Control returned an invalid controls payload.")
    ...
    if luna_control is None:
        for name, control in matched_controls.items():
            if _find_evaluator_config(_control_data(control), "galileo.luna") is not None:
                luna_control = control
                break
    if luna_control is None:
        if args.expect_luna_deny:
            raise RuntimeError("The demo agent target did not return an enabled Galileo Luna deny control.")
        luna_config = None
    else:
        luna_data = _control_data(luna_control)
        luna_action = luna_data.get("action")
        luna_decision = luna_action.get("decision") if isinstance(luna_action, dict) else None
        ...
        if luna_config is None:
            ...
            print("Warning: Luna control is present but does not include evaluator 'galileo.luna'.")
...
async def _login_with_api_key(client: httpx.AsyncClient, api_base_url: str, api_key: str) -> dict[str, str]:
...
async def _verify_runtime_token_exchange(
    ...
    *,
    ...
    async with httpx.AsyncClient(base_url=args.server_url.rstrip("/"), timeout=60.0) as client:
    ...
    if not isinstance(token, str) or not token:
        raise RuntimeError("Runtime token exchange succeeded but did not return a token.")
...
async def _verify_scorer_invoke(args: argparse.Namespace, luna_config: dict[str, Any] | None = None) -> None:
    api_key = os.environ.get("GALILEO_API_KEY")
    if not api_key:
        raise RuntimeError("Missing GALILEO_API_KEY in the environment.")
    ...
    if not isinstance(scorer_label, str) or not scorer_label:
        print()
        print("Galileo scorer invoke verification: skipped; no enabled Luna control was returned.")
        return
    ...
    print()
    print("Galileo scorer invoke verification:")
    print(f"  api_base_url={api_base_url}")
    print("  project_id=not-sent")
    print(f"  scorer_id={'sent' if body.get('scorer_id') else 'not-sent'}")
    print(f"  scorer_version_id={'sent' if body.get('scorer_version_id') else 'not-sent'}")
...
async def _verify_trace(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        partial_search_payload = await _fetch_json(
            ...
            body={
                "log_stream_id": log_stream_id,
                "filter_tree": {
                    "filter": {"name": "type", "operator": "eq", "value": "control", "type": "text"}
                },
                "pagination": {"limit": 20},
                "select_columns": {
                    "column_ids": [
    ...
    matching_partial_records = [
        record for record in partial_search_payload.get("records", []) if record.get("trace_id") == trace_id
    ]

    if control_count == 0 or not matching_partial_records:
        raise RuntimeError("Devstack verification did not return any ControlSpan records for this trace.")

    print()
    print("Galileo API verification:")
    print(f"  api_base_url={api_base_url}")
    print(f"  current_user={current_user.get('email') or current_user.get('id') or 'unknown'}")
    print(f"  ingest_health={ingest_health.get('status') or ingest_health.get('name') or 'ok'}")
    ...
    print(f"  control_spans_in_partial_search_for_trace={len(matching_partial_records)}")
...
async def _query_trends(
    ...
    *,
    ...
    if not any(_payload_has_non_zero_metrics(payload) for _, payload in results):
        raise RuntimeError("Trend queries completed, but none of the control chart payloads contained data.")
...
async def _run(args: argparse.Namespace) -> None:
    ...
    session_external_id = f"agent-control-galileo-e2e-{uuid4()}"
    session_id = logger.start_session(
        name=args.session_name,
        external_id=session_external_id,
        metadata={"demo": "agent-control-galileo-e2e"},
    )
    ...
    if args.setup_controls:
        ...
        for control_name, control_id in zip((name for name, _ in control_specs()), control_ids, strict=True):
    ...
    if not args.skip_runtime_token_check and args.runtime_auth_mode == "jwt":
        await _verify_runtime_token_exchange(args, target_type=target_type, target_id=target_id)

    agent_control.init(
        agent_name=args.agent_name,
        agent_description="Standalone banking transfer demo for Agent Control + Galileo control spans",
        server_url=args.server_url,
        api_key=resolve_agent_control_api_key(),
    ...
    trace = logger.start_trace(input=trace_input, name="agent-control-galileo-e2e")
    ...
    try:
        ...
        print()
        print(f"Galileo trace_id: {trace_id}")
